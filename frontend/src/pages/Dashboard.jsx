...
import { Download, RefreshCw, ArrowLeft, CheckCircle2, AlertTriangle, ShieldAlert } from 'lucide-react';

export default function Dashboard({ data, onReset }) {
  const evaluation = data || {
    score: 0, relevance: 0, confidence: 0,
    technical_understanding: 0, problem_solving: 0, project_quality: 0, career_readiness: 0,
    strengths: [], weaknesses: [], missing_skills: [], feedback: ""
  };
    ...
    <div className="w-full text-left space-y-8 animate-fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/[0.06] pb-6">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={onReset} className="flex items-center gap-2 px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 rounded-xl text-sm font-bold shadow-lg shadow-indigo-600/20 transition-colors cursor-pointer"><ArrowLeft className="w-4 h-4" /> Back to Dashboard</button>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6 items-start">
        <div className="p-8 rounded-2xl bg-[#0b0f19]/60 border border-white/[0.06] backdrop-blur-sm flex flex-col items-center justify-center text-center relative min-h-[320px]">
          <h3 className="text-xs font-bold tracking-wider text-slate-500 uppercase absolute top-6 left-6">Overall Evaluation</h3>
          <div className="relative w-40 h-40 flex items-center justify-center mt-6">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              ...
              <circle cx="50" cy="50" r="42" stroke="#a855f7" strokeWidth="6" fill="transparent"
                strokeDasharray={2 * Math.PI * 42} strokeDashoffset={2 * Math.PI * 42 * (1 - evaluation.score / 100)} strokeLinecap="round" />
            </svg>
            <div className="absolute">
              <span className="text-5xl font-black text-white block">{evaluation.score}</span>
          ...
          <div className="mt-6 flex gap-6 text-xs text-left w-full border-t border-white/[0.04] pt-4 px-2">
            <div><span className="text-slate-500 block">Relevance</span><span className="text-white font-bold">{evaluation.relevance}%</span></div>
            <div><span className="text-slate-500 block">AI Confidence</span><span className="text-purple-400 font-bold">{evaluation.confidence}%</span></div>
          </div>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <div className="p-6 rounded-2xl bg-amber-500/[0.02] border border-amber-500/10 space-y-4">
          <h3 className="text-sm font-bold text-amber-400 uppercase tracking-widest flex items-center gap-2"><AlertTriangle className="w-4 h-4" /> Weaknesses</h3>
          <ul className="space-y-2 text-sm text-slate-300">
            {evaluation.weaknesses.map((wk, i) => <li key={i} className="flex items-start gap-2"><span className="text-amber-500 font-bold mt-0.5">·</span><span>{wk}</span></li>)}
          </ul>
        </div>
      </div>

      <div className="p-6 rounded-2xl bg-[#0b0f19]/60 border border-white/[0.06] backdrop-blur-sm space-y-2">
        <h3 className="text-xs font-bold text-indigo-400 tracking-wider uppercase">AI Core Summary Feedback</h3>
