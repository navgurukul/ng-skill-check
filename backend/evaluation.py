...
def evaluate_submission(*, file_bytes: bytes | None = None, track: str,
                        evaluation_type: str, repo_url: str | None = None) -> dict:
    """
    Run a single candidate evaluation through the DeepSeek pipeline.

    Shared core used by both the HTTP endpoint (POST /api/evaluate) and the
    email intake worker, so both paths produce identical results.

    Raises HTTPException (400/500) on bad input or model errors, mirroring the
